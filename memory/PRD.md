# Магазин грузовых автозапчастей - PRD

## Проблема
Создание интернет-магазина грузовых автозапчастей с каталогом, корзиной, оформлением заказов и личным кабинетом.

## Архитектура
- **Frontend**: React + Tailwind CSS + Shadcn UI
- **Backend**: FastAPI + MongoDB
- **Auth**: JWT tokens (bcrypt для паролей)
- **Media Storage**: Cloudinary

## Реализованный функционал

### v1.0 - MVP (16.01.2025)
- ✅ Главная страница с hero-секцией и 6 брендами
- ✅ Каталог товаров в виде списка с модальными окнами
- ✅ Поиск по названию и артикулу
- ✅ Регистрация и авторизация пользователей
- ✅ Корзина (добавление, изменение количества, удаление)
- ✅ Оформление заказа (ФИО, адрес, телефон)
- ✅ Личный кабинет с историей заказов
- ✅ Административная панель
- ✅ Рекламный баннер с кастомизацией

### v2.0 - Расширение функционала (17.01.2025)
- ✅ **Популярные товары** — секция на главной странице
- ✅ **Сопутствующие товары** — блок в модальном окне товара
- ✅ **Избранное** — возможность сохранять товары в список желаний
- ✅ **Расширенная статистика** — графики продаж, топ товаров, топ клиентов
- ✅ **Импорт товаров из CSV** — массовая загрузка товаров
- ✅ **Экспорт товаров в CSV** — выгрузка каталога
- ✅ **Telegram уведомления** — оповещения о новых заказах
- ✅ **Встроенный онлайн-чат** — общение клиентов с поддержкой

### v3.0 - Фаза 1 расширенного админ-функционала (26.01.2025)
- ✅ **/api/health endpoint** — для деплоя на Kubernetes
- ✅ **Полное управление пользователями** — CRUD в админке с отображением пароля
- ✅ **Полное управление заказами** — редактирование и удаление заказов без перезагрузки
- ✅ **Кросс-номера для товаров** — поле cross_articles в форме редактирования
- ✅ **Расширенный поиск** — разделение на "Точное совпадение" и "Возможные замены"

### v3.1 - Фаза 2 Real-time и мультифото (26.01.2025)
- ✅ **Real-time обновления** — все действия в админке (товары, категории) обновляют UI без перезагрузки
- ✅ **Мультифото для товаров** — загрузка нескольких изображений с галереей в админке
- ✅ **Слайдшоу в каталоге** — навигация по изображениям в модальном окне товара с миниатюрами

### v3.2 - Фаза 3 Telegram-чат и управление (26.01.2025)
- ✅ **Telegram-бот для чата** — двусторонняя связь админа через Telegram (/reply, /chats, /start)
- ✅ **Улучшенный чат-виджет** — загрузка медиафайлов, эмодзи-пикер (112 эмодзи), gradient дизайн
- ✅ **Управление чатами в админке** — закрепление, метки, удаление чатов
- ✅ **Редактирование сообщений** — админ может редактировать/удалять свои сообщения

### v3.3 - Фаза 5 Бонусная программа V2 (31.01.2025)
- ✅ **Полностью переработанная бонусная система** — новые требования пользователя
- ✅ **Редактируемые настройки** — заголовок, описание, изображение программы
- ✅ **Прогресс по доставленным заказам** — бонусы начисляются только при статусе "доставлен"
- ✅ **Запрос бонуса пользователем** — кнопка появляется при достижении минимального порога (5000 ₽)
- ✅ **Выдача бонуса через промокод** — админ вводит промокод в модальном окне
- ✅ **Сброс прогресса** — только после выдачи бонуса админом
- ✅ **Индикатор запросов** — пользователи с запросом выделены в админке

### v3.4 - Множественные бонусные программы (31.01.2025)
- ✅ **Неограниченное количество программ** — создание независимых бонусных программ
- ✅ **Индивидуальные настройки** — каждая программа имеет свои параметры
- ✅ **Типы начисления** — 100% от заказа или процент (настраиваемый)
- ✅ **Независимый прогресс** — отслеживание по каждой программе отдельно
- ✅ **Одновременные запросы** — пользователь может запросить бонус из нескольких программ
- ✅ **CRUD операции** — создание, редактирование, удаление программ в админке
- ✅ **Раскрываемый список пользователей** — для каждой программы

### v3.5 - Расширенный профиль и статистика (31.01.2025)
- ✅ **Новый дизайн профиля** — крупный тёмный header с фото, ФИО, контактами
- ✅ **Полное редактирование профиля** — email, пароль, телефон, адрес, ФИО, фото
- ✅ **Смена пароля** — с проверкой текущего пароля
- ✅ **Смена email** — с проверкой уникальности
- ✅ **Расширенная статистика заказов** — доставлено/в работе, виды товаров, дата регистрации
- ✅ **Популярные товары** — топ-5 часто заказываемых товаров
- ✅ **График по месяцам** — 12 месяцев истории заказов

### v3.6 - Раскрывающиеся карточки пользователей (31.01.2025)
- ✅ **Карточки пользователей** — аватар, имя, роль, email, быстрая статистика
- ✅ **Раскрывающиеся детали** — клик на карточку раскрывает полную информацию
- ✅ **Личные данные** — ФИО, email, пароль (видимый), телефон, адрес, роль, дата регистрации
- ✅ **Статистика заказов** — количество, сумма, средний чек, товаров куплено
- ✅ **По статусам** — распределение заказов по статусам
- ✅ **Популярные товары** — топ товаров пользователя
- ✅ **Последние заказы** — последние 3 заказа с суммой и статусом

### v3.7 - Расширенная аналитика (31.01.2025)
- ✅ **Градиентные KPI-карточки** — выручка, заказы, средний чек, пользователи
- ✅ **Сравнение с прошлым периодом** — процент роста/падения
- ✅ **Вторичная статистика** — товары, заказы всего, конверсия, заказов на клиента
- ✅ **График продаж по дням** — с тултипами и суммами
- ✅ **Активность по часам** — распределение заказов 0-24 часа
- ✅ **Статусы заказов** — цветные бейджи с процентами
- ✅ **Топ товаров** — по выручке с артикулами
- ✅ **Топ клиентов** — по потраченной сумме
- ✅ **Топ производителей** — по продажам
- ✅ **Выбор периода** — день/неделя/месяц/год

### v3.4 - Фаза 4 Партнёры и профиль (31.01.2025)
- ✅ **Редактируемые партнёры** — CRUD в админке, отображение на главной странице
- ✅ **Фото профиля** — загрузка/удаление аватара пользователями
- ✅ **Статистика заказов** — карточки метрик, диаграмма по статусам, график по месяцам

### v3.8 - Редизайн бонусной программы и оптимизация админ-панели (05.02.2025)
- ✅ **Отдельная карточка баллов** — оранжевая карточка "Ваши накопленные баллы" на странице аккаунта
- ✅ **Объединенный блок уровней и прогресса** — уровни программы (Бронза→Серебро→Золото→Платина) и прогресс-бар в одном блоке
- ✅ **Динамический прогресс-бар** — показывает сумму покупок за год и сколько осталось до следующего уровня (напр. "до Серебро: 10 000 ₽")
- ✅ **Real-time обновления в админке** — все CRUD операции (товары, бонусные программы, партнёры) обновляют UI без перезагрузки страницы
- ✅ **Оптимизированные state-updates** — замена fetchData() на локальные обновления состояния (setProducts, setBonusPrograms, setPartners)

### v3.9 - Разделение бонусов и профиля пользователя (06.02.2025)
- ✅ **Отдельная страница бонусов /bonus** — выделена из профиля пользователя в самостоятельную страницу
- ✅ **Красивый дизайн страницы бонусов** — оранжевая карточка с баллами, визуализация уровней, раскрывающийся список призов
- ✅ **Упрощённая страница профиля /account** — только редактирование личных данных (имя, телефон, адрес) и смена пароля
- ✅ **Навигация "Бонусы" в шапке** — оранжевая кнопка с иконкой подарка для пользователей
- ✅ **Редирект админа** — админ с /account автоматически перенаправляется на /admin/profile
- ✅ **Современный дизайн профиля** — градиентный header с аватаром, карточка безопасности для смены пароля

## Структура проекта
```
/app
├── backend/
│   ├── .env
│   ├── requirements.txt
│   ├── server.py              # FastAPI app, all API endpoints
│   ├── cloudinary_service.py  # Cloudinary integration for media storage
│   └── uploads/               # Local uploads (legacy)
└── frontend/
    ├── src/
    │   ├── components/
    │   │   ├── ChatWidget.js      # Виджет онлайн-чата с lightbox
    │   │   ├── FavoritesButton.js # Кнопка добавления в избранное
    │   │   ├── Header.js
    │   │   ├── PopularProducts.js # Секция популярных товаров
    │   │   ├── ProductCard.js
    │   │   ├── PromoBanner.js
    │   │   ├── RelatedProducts.js # Сопутствующие товары
    │   │   └── ui/
    │   ├── context/
    │   │   ├── AuthContext.js
    │   │   └── CartContext.js
    │   └── pages/
    │       ├── AccountPage.js     # + Редизайн бонусов (точки, годовой прогресс)
    │       ├── AdminPage.js       # + Real-time CRUD, все вкладки без перезагрузки
    │       ├── CartPage.js
    │       ├── CatalogPage.js     # + Избранное, Сопутствующие
    │       ├── CheckoutPage.js
    │       ├── FavoritesPage.js   # Страница избранного
    │       ├── HomePage.js        # + Популярные товары
    │       ├── LoginPage.js
    │       └── RegisterPage.js
```

## API Endpoints

### Аутентификация
- POST /api/auth/register
- POST /api/auth/login
- GET /api/auth/me

### Товары
- GET /api/products
- GET /api/products/popular
- GET /api/products/{id}
- GET /api/products/{id}/related
- POST /api/products (admin)
- PUT /api/products/{id} (admin)
- DELETE /api/products/{id} (admin)

### Категории
- GET /api/categories
- POST /api/categories (admin)
- PUT /api/categories/{id} (admin)
- DELETE /api/categories/{id} (admin)

### Корзина
- GET /api/cart
- POST /api/cart/add
- POST /api/cart/update
- DELETE /api/cart/{product_id}
- DELETE /api/cart

### Заказы
- GET /api/orders
- POST /api/orders
- GET /api/orders/{id}

### Избранное
- GET /api/favorites
- POST /api/favorites/add
- DELETE /api/favorites/{product_id}
- GET /api/favorites/check/{product_id}

### Чат
- GET /api/chat/messages
- POST /api/chat/send
- GET /api/chat/unread-count
- POST /api/chat/mark-read

### Администрирование
- GET /api/health — health check для K8s
- GET /api/admin/stats
- GET /api/admin/stats/extended
- **GET /api/admin/users** — список пользователей с password_plain
- **POST /api/admin/users** — создание пользователя
- **PUT /api/admin/users/{id}** — редактирование пользователя
- **DELETE /api/admin/users/{id}** — удаление пользователя
- GET /api/admin/orders
- **PUT /api/admin/orders/{id}** — редактирование заказа
- PUT /api/admin/orders/{id}/status
- **DELETE /api/admin/orders/{id}** — удаление заказа
- GET /api/admin/chats
- GET /api/admin/chats/{id}/messages
- POST /api/admin/chats/{id}/send
- GET /api/admin/telegram-settings
- PUT /api/admin/telegram-settings
- POST /api/admin/telegram-test
- POST /api/admin/products/import
- GET /api/admin/products/export

### Товары (расширено)
- **GET /api/products/search-with-alternatives** — поиск с кросс-номерами

### Бонусная программа
- GET /api/bonus/programs — список программ с yearly_total, bonus_points, levels, prizes
- POST /api/bonus/request/{program_id} — запросить бонус
- POST /api/bonus/redeem-prize — потратить баллы на приз
- GET /api/bonus/history — история полученных промокодов
- GET /api/admin/bonus/programs — список программ (admin)
- POST /api/admin/bonus/programs — создать программу (admin)
- PUT /api/admin/bonus/programs/{id} — обновить программу (admin)
- DELETE /api/admin/bonus/programs/{id} — удалить программу (admin)
- GET /api/admin/bonus/programs/{id}/users — пользователи программы (admin)
- POST /api/admin/bonus/programs/{id}/issue/{user_id} — выдать промокод (admin)
- GET /api/admin/bonus/history — история выдачи (admin)

### Прочее
- GET /api/promo-banner
- PUT /api/promo-banner (admin)
- POST /api/upload (admin)
- POST /api/seed

## Учётные данные
- **Admin**: admin@avarus.ru / admin123
- **Test User**: testbonus@test.com / test123

## Backlog (P0 - Ближайшие задачи)
- Пагинация и поиск в списке пользователей в админ-панели
- Экспорт статистики заказов (Excel/PDF)
- Telegram-уведомление при выдаче промокода

## Backlog (P1)
- Уведомления о наличии товаров
- Калькулятор доставки по регионам
- Email уведомления о заказах

## Backlog (P2)
- Интеграция с 1С
- Фильтр по марке/модели грузовика
- Отзывы о товарах

